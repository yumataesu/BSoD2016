...
void ofxPostGlitch::generateFx(){
	...
	for (int i = 0;i < GLITCH_NUM;i++){
		if (bShading[i]){
            ...
            shader[i].setUniform1i("ksectors", 10);
            shader[i].setUniform1f("kangleRad", ofDegToRad(0.0));
            shader[i].setUniform2f("kcenter", 0.5*512.0, 0.5*512.0);
            shader[i].setUniform2f("screenCenter", 0.5*ofGetWidth(), 0.5*ofGetHeight());
